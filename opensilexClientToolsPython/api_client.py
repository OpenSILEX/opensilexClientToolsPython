...
import opensilexClientToolsPython.models
...
class ApiClient(object):
    ...
    def __init__(self, configuration=None, header_name=None, header_value=None,
        ...
        self.user_agent = 'Swagger-Codegen/1.4.6/python'
    ...
    def __call_api(
        ...
        if _preload_content:
            ...
            if response_type:
                return_data = self.deserialize(response_data, response_type,method)                
            else:
                return_data = None
                if(resource_path == "/brapi/v1/token"):
                    return_data = json.loads(response_data.data)
            
            return_data_dict = {}
            # with json
            if header_params['Content-Type'] != "application/octet-stream":
        ...
        if _return_http_data_only:
            ...
            return_data_dict['headers']= response_data.getheaders()
            return_data_dict['status']= response_data.status
            return (return_data_dict)
    ...
    def deserialize(self, response, response_type,method):
        ...
        :param method: string which define http request verb.
        ...
        try:
            ...
            if method == "GET":
                data = data["result"]
            if method == "POST":
                if(data["metadata"]["datafiles"] == []):
                    data = data["result"] 
                else:
                    return data["metadata"]["datafiles"]
            if method == "PUT":
               return data["metadata"]["datafiles"]
        except ValueError:
            data = response.data
    ...
    def __deserialize(self, data, klass):
        ...
        if type(klass) == str:
            ...
            if klass in self.NATIVE_TYPES_MAPPING:
                ...
                # Special case of BrAPI list responses.
                if "ListResponse" in klass:
                    intermediary_klass = getattr(opensilexClientToolsPython.models, klass)
                    klass = getattr(opensilexClientToolsPython.models,intermediary_klass.swagger_types["result"])
                else:
                    klass = getattr(opensilexClientToolsPython.models, klass)
    ...
    def connect_to_opensilex_ws(self, identifier=None, password=None, host = None):
            ...
            try:
                token_api_response = token_api_instance.authenticate(
                    body=token_request)
                self.default_headers["Authorization"] = "Bearer " + \
                    token_api_response['result'].token
